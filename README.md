...
![Architectural Diagram]("https://code.amazon.com/packages/Aws-serverless-telepresence-robot/blobs/heads/master/--/diagram.png?raw=1")
...
```bash
telepresence-robot$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
telepresence-robot$ sam local start-api
telepresence-robot$ curl http://localhost:3000/
```
...
```bash
telepresence-robot$ sam logs -n HelloWorldFunction --stack-name telepresence-robot --tail
```
...
```bash
telepresence-robot$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name telepresence-robot
```
